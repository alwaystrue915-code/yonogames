...
export const AppCard: React.FC<AppCardProps> = ({ app, variant = 'medium', rank, onSelect, cardStyle = 'default' }) => {
    ...
    return (
      ...
      >
        ...
        {rank !== undefined && rank <= 3 && (
          <div className="absolute -top-3 left-1/2 -translate-x-1/2 flex items-center justify-center z-10 select-none">
            <span className={`text-[9.5px] font-black px-2.5 py-0.5 rounded-full shadow-sm text-white flex items-center gap-0.5 whitespace-nowrap uppercase tracking-wide ${
              ...
              {isRank1 ? <><Crown size={10} /> RANK 1</> : isRank2 ? <><Medal size={10} /> RANK 2</> : <><Medal size={10} /> RANK 3</>}
            </span>
          </div>
        )}
        ...
        <div className="w-full flex flex-col gap-1.5 mt-3 shrink-0">
          <span
            className="w-full py-2.5 rounded-xl text-center text-[11px] font-black flex items-center justify-center gap-1 cursor-pointer transition-all duration-200 bg-[#dc2626] text-white shadow-[0_4px_12px_rgba(220,38,38,0.25)] hover:bg-[#b91c1c] hover:shadow-[0_6px_16px_rgba(220,38,38,0.35)] active:scale-[0.97]"
          >
  ...
  let cardClass = "flex items-center gap-3 py-2 px-2.5 lg:py-0 lg:px-0 rounded-xl transition-all cursor-pointer border-b border-slate-100 last:border-b-0 lg:border-b-0 bg-white relative lg:border lg:border-slate-200 lg:rounded-2xl lg:shadow-sm lg:hover:shadow-md lg:overflow-hidden";
  ...
  if (cardStyle === 'dynamic-border') {
    ...
    // default
    cardClass = "flex items-center gap-3 py-2 px-2.5 lg:py-0 lg:px-0 hover:bg-slate-50/70 lg:hover:bg-white rounded-xl transition-all cursor-pointer border-b border-slate-100 last:border-b-0 lg:border-b-0 bg-white relative lg:border lg:border-slate-200 lg:rounded-2xl lg:shadow-sm lg:hover:shadow-md lg:overflow-hidden";
  }
  ...
  return (
    <Link prefetch={true}
      ...
      className={`${cardClass} text-inherit no-underline lg:flex-col lg:items-stretch`}
    >
      ...
      <div className="flex items-center gap-3 flex-1 lg:flex-col lg:items-center lg:text-center lg:pt-4 lg:px-3">
        {/* Rank indicator */}
        {rank !== undefined && (
          <div className="w-5 text-center font-bold text-sm text-slate-400 relative z-10 lg:hidden">
            {rank}
          </div>
        )}
        ...
        <div className="flex-1 min-w-0 relative z-10 font-sans text-left lg:text-center">
        ...
        <div className="flex items-center gap-2 shrink-0 relative z-10 lg:hidden">
          <span className="px-4 py-2 rounded-xl text-xs font-black flex items-center gap-1.5 cursor-pointer transition-all duration-200 bg-[#dc2626] text-white shadow-[0_4px_12px_rgba(220,38,38,0.22)] hover:bg-[#b91c1c] hover:shadow-[0_6px_16px_rgba(220,38,38,0.32)] active:scale-[0.97]">
            <Download size={13} /> Download
          </span>
        </div>
      </div>

      {/* Action triggers - desktop (full width button at bottom) */}
      <div className="hidden lg:block px-3 pb-3">
        <span className="w-full py-2.5 rounded-xl text-xs font-black flex items-center justify-center gap-1.5 cursor-pointer transition-all duration-200 bg-[#dc2626] text-white shadow-[0_4px_12px_rgba(220,38,38,0.22)] hover:bg-[#b91c1c] hover:shadow-[0_6px_16px_rgba(220,38,38,0.32)] active:scale-[0.97]">
          <Download size={13} /> Download
        </span>
      </div>
    </Link>
